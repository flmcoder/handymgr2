// ============================================================================
// main.ts — HandyManager Proxy v9.7.3
...
import { getFetchResilienceTelemetry } from "./lib/fetchUtils.ts";
...
import {
  ...
  handleBills,
  handleBillsRoute,
  handleBillingSavedReport,
} from "./handlers/bills.ts";
...
} from "./handlers/magicPortal.ts";
import * as tenantComms from "./handlers/tenantComms.ts";
import {
...
let _schemaBootstrapReady = false;
let _schemaBootstrapInFlight: Promise<void> | null = null;
let _schemaBootstrapLastAttemptMs = 0;
let _schemaBootstrapLastError = "";
const SCHEMA_BOOTSTRAP_RETRY_MS = 30_000;
...
async function ensureCriticalTables(): Promise<void> {
  const statements: string[] = [
    ...
    `CREATE TABLE IF NOT EXISTS routing_monitor_capabilities (
    ...
    `CREATE TABLE IF NOT EXISTS routing_monitor_pm_map (
       id         INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    `CREATE TABLE IF NOT EXISTS routing_monitor_events (
       id            INTEGER PRIMARY KEY AUTOINCREMENT,
       event_uuid    TEXT,
       wo_id         TEXT,
       vendor_id     TEXT,
...
async function ensureSchemaBootstrap(): Promise<void> {
  if (_schemaBootstrapReady) return;
  const now = Date.now();
  if (_schemaBootstrapInFlight) {
    await _schemaBootstrapInFlight;
    return;
  }
  if (_schemaBootstrapLastAttemptMs > 0 &&
    (now - _schemaBootstrapLastAttemptMs) < SCHEMA_BOOTSTRAP_RETRY_MS
  ) {
    return;
  }

  _schemaBootstrapLastAttemptMs = now;
  _schemaBootstrapInFlight = (async () => {
    try {
      await ensureTables();
      await ensureCriticalTables();
      _schemaBootstrapReady = true;
      _schemaBootstrapLastError = "";
    } catch (e: any) {
      if (!isSqlWriteBlockedError(e)) throw e;
      _schemaBootstrapLastError = String(e?.message || e || "write blocked");
      console.warn(
        "[SCHEMA WARNING] bootstrap skipped: SQL writes blocked by plan/quota",
      );
    } finally {
      _schemaBootstrapInFlight = null;
    }
  })();
...
function missingTenantCommsExport(name: string) {
  return async () => ({
    ok: false,
    status: 501,
    error: "tenant_comms_export_missing",
    message:
      `The deployed tenantComms module is missing required export: ${name}. ` +
      "Redeploy with matching main.ts and handlers/tenantComms.ts versions.",
  });
}

const handleAddMonitoredWO = tenantComms.handleAddMonitoredWO ??
  missingTenantCommsExport("handleAddMonitoredWO");
const handleGenerateMagicLink = tenantComms.handleGenerateMagicLink ??
  missingTenantCommsExport("handleGenerateMagicLink");
const handlePortalNoContact = tenantComms.handlePortalNoContact ??
  missingTenantCommsExport("handlePortalNoContact");
const handlePortalNote = tenantComms.handlePortalNote ??
  missingTenantCommsExport("handlePortalNote");
const handlePortalPhotoUpload = tenantComms.handlePortalPhotoUpload ??
  missingTenantCommsExport("handlePortalPhotoUpload");
const handlePortalReassignRequest = tenantComms.handlePortalReassignRequest ??
  missingTenantCommsExport("handlePortalReassignRequest");
const handlePortalReschedule = tenantComms.handlePortalReschedule ??
  missingTenantCommsExport("handlePortalReschedule");
const handlePortalSchedule = tenantComms.handlePortalSchedule ??
  missingTenantCommsExport("handlePortalSchedule");
const handlePortalStatus = tenantComms.handlePortalStatus ??
  missingTenantCommsExport("handlePortalStatus");
const handlePortalValidate = tenantComms.handlePortalValidate ??
  missingTenantCommsExport("handlePortalValidate");
const handleRemoveMonitoredWO = tenantComms.handleRemoveMonitoredWO ??
  missingTenantCommsExport("handleRemoveMonitoredWO");
const handleSendMagicLinkTestSMS = tenantComms.handleSendMagicLinkTestSMS ??
  missingTenantCommsExport("handleSendMagicLinkTestSMS");
const handleSendTenantSMS = tenantComms.handleSendTenantSMS ??
  missingTenantCommsExport("handleSendTenantSMS");
const handleTenantCommsLog = tenantComms.handleTenantCommsLog ??
  missingTenantCommsExport("handleTenantCommsLog");
...
function compactBackgroundTasks(): void {
  if (backgroundTasks.size <= MAX_BACKGROUND_TASKS) return;
  const sorted = Array.from(backgroundTasks.values())
    .sort((a, b) =>
      String(a.started_at || "").localeCompare(String(b.started_at || ""))
    );
  const remove = sorted.slice(0, backgroundTasks.size - MAX_BACKGROUND_TASKS);
  for (const task of remove) backgroundTasks.delete(task.id);
}
...
function startBackgroundTask(
  ...
  queueMicrotask(async () => {
    ...
    try {
      ...
      task.summary = {
        ok: !!result?.ok,
        count: Number(result?.count || result?.upserted || result?.synced || 0),
      };
    } catch (err: any) {
      task.status = "failed";
      task.error = String(err?.message || err || "task failed").slice(0, 300);
    } finally {
...
const ENFORCE_FRONTEND_AUTH = envFlag("ENFORCE_FRONTEND_AUTH", true);
...
async function resolveTrustedDeviceSession(
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    if (i < attempts - 1) {
      await delayMs(Math.min(80 * (i + 1), 320));
    }
  }
  return null;
}
...
export default async function handler(req: Request): Promise<Response> {
  ...
  let action = "";
  try {
    // ── Ensure all SQLite/Turso tables exist (warm once, then fast no-op) ───
    try {
      await Promise.race([
      ...
      console.warn(
        "[SCHEMA WARNING] schema bootstrap skipped: SQL writes blocked by plan/quota",
      );
    }
    ...
    if (!action && req.method === "POST") {
      const adminPathToAction: Record<string, string> = {
        ...
        "/admin/sync/work-orders/manual-seed": "manual_seed_work_orders",
    ...
    if (req.method === "GET" && rawPath.startsWith("/s/")) {
      ...
      const shortCode = shortLinkMatch ? shortLinkMatch[1] : "";
      return await handleShortLinkRedirect(req, shortCode);
    }
    ...
    if (action === "system_health") {
      const effectiveRole = String(
        frontendSession?.role ||
          (frontendTokenMatchesSharedSecret ? "full" : ""),
      ).toLowerCase();
      const allowed =
        isHeaderAdminAuthorized ||
        effectiveRole === "full" ||
      ...
      if (!allowed) {
        return jsonResp({
          ok: false,
          error:
            "Forbidden: system_health is limited to manager/admin sessions",
        }, 403);
      }
    }
    ...
    ) ? String(frontendSession.property_group_uuid) : undefined;

    const SCOPED_DATA_ACTIONS = new Set([
      "work_orders",
      "manual_seed_work_orders",
    ...
    if (req.method === "POST" && action) {
      ...
      switch (action) {
        ...
        case "bill_attachment_upload":
          ...
          break;
        case "aio_bills":
          result = await handleAioBillSearch(req);
        ...
        case "unit_turns_sync":
          if (["1", "true", "yes"].includes(
            String(params.async || "").toLowerCase(),
          )) {
            const clonedReq = req.clone();
            const task = startBackgroundTask(
              "unit_turns_sync",
              async () => await handleUnitTurnsSync(clonedReq),
            );
            return jsonResp({
              ok: true,
              accepted: true,
              ...
              message: "unit_turns_sync accepted for background processing",
            }, 202);
          }
        ...
        case "admin_resolve_groups":
          ...
          break;
        case "manual_seed_work_orders":
          result = await handleManualSeedWorkOrders(params);
          break;
      ...
      return jsonResp(result, Number.isFinite(status) ? status : 200);
    }
    ...
    if (req.method === "GET" || req.method === "HEAD") {
      ...
      switch (action) {
        ...
        // ── Bills ─────────────────────────────────────────────────────────────
        case "billing_saved_report":
          return handleBillingSavedReport();
        case "v2_report": {
          const reportName = String(params.report || "").trim().replace(/[^a-z0-9_]/g, "");
          if (!reportName) {
            return jsonResp({ ok: false, error: "Missing report name" }, 400);
          }
          return handleV2Report(reportName, params, sessionScopeUuid);
        }
        case "bills":
        case "bills_stats":
        case "bill_detail":
        ...
        case "bills_list_v2":
        case "bills_parity_check":
          result = await handleBills(params, req, action);
          break;
        case "bill_detail_v0": {
          const billId = String(params.bill_id || params.id || "").trim();
          result = await handleBillDetailV0(req, billId);
          if (result?.status && result.status !== 200) {
            return jsonResp(result, result.status);
          }
          break;
        ...
        case "bills_sync":
          if (["1", "true", "yes"].includes(
            ...
            const task = startBackgroundTask(
              "bills_sync",
              async () => await handleBills(params, req, "bills_sync"),
            );
            result = {
              ok: true,
              accepted: true,
              ...
              task_id: task.id,
              task_status: task.status,
              message: "bills_sync accepted for background processing",
            };
            break;
          }
          result = await handleBills(params, req, action);
        ...
        case "resilience_telemetry":
          result = {
            ok: true,
            telemetry: getFetchResilienceTelemetry(),
            background_tasks: backgroundTasks.size,
          };
          break;
          ...
          result = {
            ...
            endpoints: {
              ...
              work_orders: [
                ...
                "POST ?action=manual_seed_work_orders&days=14&async=1",
              ...
              turns_inspections: [
                ...
                "POST ?action=unit_turns_sync&async=true  body: {records:[...]}",
                ...
                "GET  ?action=bills_sync&async=true",
                "GET  ?action=bills_history&days=365",
                "GET  ?action=sync_task_status&task_id=UUID",
                "GET  ?action=resilience_telemetry",
                "GET  ?action=bills_parity_check&group_id=UUID&date_from=YYYY-MM-DD&date_to=YYYY-MM-DD&status=all",

...
import {
  propertyInScope,
  resolveGroupPropertyIds,
  unitToPropertyId,
} from "../lib/groupUtils.ts";
...
async function resolveWoUuidFromReference(
  ...
  const normalizedCandidates = [
    ref,
    String(ref).replace(/^WO[-\s]?/i, "").trim(),
    String(ref).replace(/[^0-9]/g, "").trim(),
    ...
    const linkRows = rowsAsObjects(
      await sqlite.execute({
        sql: `SELECT wo_db_uuid
            ...
            WHERE wo_id IN (${normalizedCandidates.map(() => "?").join(",")})
              AND wo_db_uuid IS NOT NULL
              AND wo_db_uuid <> ''
  ...
  try {
    for (const candidateRef of normalizedCandidates) {
      const dbPath = `/api/v0/work_orders?filters[WorkOrderNumber]=${
        encodeURIComponent(candidateRef)
      }&filters[LastUpdatedAtFrom]=2020-01-01T00:00:00Z&page[size]=5`;
      const resp = await fetchWithTimeout(`${AF_DB}${dbPath}`, {
        headers: dbHeaders(),
      });
      if (resp.ok) {
        ...
        const match = items.find((r: any) =>
          normalizedCandidates.includes(
            String(r?.WorkOrderNumber || r?.work_order_number || "").trim(),
          )
        ) || items[0];
        ...
        if (isUuid(candidate)) {
          ...
          const woNum = String(
            match?.WorkOrderNumber || match?.work_order_number || candidateRef,
          );
          sqlite.execute({
...
export async function handleWoAttachments(
  ...
  const force =
    String(params.force || params.force_refresh || "").toLowerCase() ===
      "true";
  if (!force) {
    const cached = await cacheGet(cacheKey, "work_orders");
...
export async function handleWoNotes(
  ...
  try {
    ...
    if (notesResp.status === 404) {
      const legacyPath = `/api/v0/work_order_notes?filters[WorkOrderId]=${
        encodeURIComponent(woId)
      }&page[size]=200`;
...
export async function handleTurnWorkOrders(
  ...
  const resolveScopedPropertyId = async (wo: any): Promise<string> => {
    const explicitPropertyId = String(wo?.PropertyId || wo?.property_id || "")
      .trim();
  ...
  const filterByGroupScope = async (
    rows: any[],
  ...
  const turnWOs = rows.filter((wo: any) => {
    const cat = String(
      wo?.Category || wo?.category || wo?.Type || wo?.type || "",
    ).trim().toLowerCase();

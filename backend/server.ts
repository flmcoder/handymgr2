...
app.get('/api/local/badge_counts', async (req: Request, res: Response) => {
  try {
    ...
    const readyToCloseRows = (turnRows as any[]).filter((row) => !!row.ready_to_close && String(row?.status || '').toLowerCase() !== 'completed');
    const priorityTurnRows = (turnRows as any[]).filter((row) => !!row.is_priority);
    ...
    // Add priority/ready metrics for dashboard attention (non-breaking)
    const now = new Date();
    ...
    const payload: any = {
      ...
      turns_all_active: activeTurnRows.length,
    };
    res.json(payload);
  } catch (error) {
...
app.get('/api/local/v2/turns', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const results = (rows as any[]).map((row) => ({
      ...
      // compat for frontend that used is_ready_to_close / is_priority_turn / elapsed_days
      is_ready_to_close: !!(row.ready_to_close ?? row.is_ready_to_close),
      is_priority_turn: !!(row.is_priority ?? row.is_priority_turn),
      elapsed_days: row.elapsed_days != null ? Number(row.elapsed_days) : null,
      rent_ready: !!(row.rent_ready ?? row.rentReady),
      has_next_resident: !!(row.has_next_resident ?? (row.next_move_in_date != null)),

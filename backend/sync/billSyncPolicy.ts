...
export function normalizeBillSyncRow(value: unknown): NormalizedBillSyncRow | null {
  ...
  return {
    ...
    status: optionalString(row.ApprovalStatus ?? row.Status ?? row.status),
    statusLabel: optionalString(row.ApprovalStatus ?? row.approval_status ?? row.StatusLabel ?? row.status_label),
    billTotalAmount: optionalNumber(row.TotalAmount ?? row.total_amount ?? row.BillTotalAmount ?? row.bill_total_amount ?? row.Amount ?? row.amount),

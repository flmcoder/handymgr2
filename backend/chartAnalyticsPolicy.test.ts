...
test('work-orders analytics scopes by property group set with one bind param', () => {
  const scope = 'bee73529-9eca-11ee-8b51-02167481f3bc';
  const { sql, params } = buildWorkOrdersAnalyticsQuery([scope, 'other-group']);
  assert.deepEqual(params, [[scope, 'other-group']]);
  assert.match(sql, /wo\.property_group_id = ANY\(\$1::text\[\]\)/);
});

test('inspections analytics encodes the missing move-in definition', () => {
  const { sql, params } = buildInspectionsAnalyticsQuery([]);
  assert.equal(params.length, 0);
...
test('inspections analytics scopes through the numeric link and group array', () => {
  const scope = 'bee73529-9eca-11ee-8b51-02167481f3bc';
  const { sql, params } = buildInspectionsAnalyticsQuery([scope, 'other-group']);
  assert.deepEqual(params, [[scope, 'other-group']]);
  assert.match(sql, /raw_json->>'Link'/);
  assert.match(sql, /PropertyGroupIds'\s*\?\|\s*\(\$1::text\[\]\)/);
});

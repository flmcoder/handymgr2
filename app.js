...
async function decryptVault(passphrase) {
  ...
  var salt = b64ToU8(VAULT_BLOB.s);
  var iv = b64ToU8(VAULT_BLOB.i);
  var tag = b64ToU8(VAULT_BLOB.t);
  var ciphertext = b64ToU8(VAULT_BLOB.c);
...
function lockVault() {
  ...
  WORK_ORDERS = []; VENDORS = []; BILLS = []; PROPERTIES = []; TURNS = []; INSPECTIONS = []; API_ERRORS = [];
...
$('#vaultUnlockBtn').addEventListener('click', async function() {
  ...
  try {
    ...
    btn.disabled = false;
    btn.innerHTML = '<i class="fas fa-lock-open"></i> Decrypt & Connect';
  }
});
...
function renderPayroll() {
  ...
  if (totalSub) totalSub.textContent = totalAmt > 0 ? 'total labor + materials' : 'no amounts recorded';
...
function renderActivityFeed() {
  ...
  // Work Order events — sorted by most recent update
  var sortedWOs = WORK_ORDERS.slice().sort(function(a, b) {
    return new Date(b.created || 0) - new Date(a.created || 0);
  });
  sortedWOs.slice(0, 30).forEach(function(wo) {
...
function getFilteredWOs() {
  ...
  return WORK_ORDERS.filter(function(wo) {
    ...
    if (currentPropertyGroup) {
      var prop = PROPERTIES.find(function(p) { return p.name === wo.propertyName; });
      if (!prop || prop.portfolio !== currentPropertyGroup) return false;
    }
    // Flagged filter
...
async function fetchAllLive() {
  ...
  updateCacheBadge('loading');
  var steps = ['Work Orders', 'Properties', 'Vendors', 'Turns', 'Inspections'];
  showProgress('Syncing AppFolio via Proxy v4 (' + DATA_WINDOW_DAYS + 'd)', steps);
